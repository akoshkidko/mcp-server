...
**Проблема:** разработчик получает незнакомый репозиторий и не знает, с чего начать — везде хаос: непонятно, сколько технического долга, почему упал CI, можно ли доверять зависимостям.

**Что демонстрирует сценарий:** за 4 вызова инструментов сервер строит полную картину состояния репозитория:
1. находит 9 аннотаций технического долга, из них 5 критичных и высоких — включая обход авторизации и токен без срока действия;
2. диагностирует упавший тест с точностью 95% и находит строку `assert 500 == 200`;
3. обнаруживает 7 незакреплённых зависимостей и 1 пакет с неизвестной лицензией;
4. агрегирует всё в health score 0.657 со статусом `needs_attention` и конкретным списком рекомендаций.

Весь анализ занимает меньше минуты — без чтения кода вручную.
